...
import "@/app/form.css";
import "@/app/toastAlerts.css"; // CSS for toast alerts
...
const XCommunityForm: React.FC<{
  ...
  const fetchCommunityData = async (url: string) => {
    try {
      const res = await fetch("https://xboard-production.up.railway.app/scrape-image", {
        method: "POST",
  ...
  // Form submit handler with URL validation
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate the community URL before proceeding
    ...
    try {
      ...
      // If the scraper cannot find the community name, show an error and stop.
      if (!communityName) {
        onShowToast(
          "error",
          "Community Not Found",
...
export default function Dashboard() {
  ...
  // Calculate remaining tags for edit form
  const editRemainingTags = MAX_TAGS - editTags.length;

  // Calculate remaining characters for edit form
  ...
  return (
    <div className="dashboard-container">
      ...
      <div style={{ textAlign: "center", margin: "20px 0" }}>
        <h2 style={{ display: "inline-block", borderBottom: "3px solid #0070f3", paddingBottom: "5px" }}>Your Dashboard</h2>
      </div>
